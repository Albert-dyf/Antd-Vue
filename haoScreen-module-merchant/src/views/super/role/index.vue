...
import { getRoles, addRole, getRoleById, updateRole, deleteRole, getRoleModuleConfig, updateRoleModuleAuthorized } from '@/api/super'
...
export default {
  ...
  methods: {
    ...
    _getRoleInfoById() {
      getRoleById(this.current.id).then(res => {
        if (res.code === 200) {
    ...
    async _getMenuConfigByRole() {
      await getRoleModuleConfig(this.current.id).then(res => {
        if (res.code === 200) {
    ...
    _updateRoleModuleAuthorized(moduleItem) {
      ...
      updateRoleModuleAuthorized(data).then(res => {
        if (res.code === 200) {
    ...
    _deleteRole() {
      deleteRole(this.current.id).then(res => {
        if (res.code === 200) {
