...
import { getModules, addModule, getModuleById, updateModule, deleteModule } from '@/api/super'
...
export default {
  ...
  methods: {
    ...
    _deleteModule() {
      deleteModule(this.current.id).then(res => {
        if (res.code === 200) {
    ...
    async _getModuleById() {
      await getModuleById(this.current.id).then(res => {
        if (res.code === 200) {
